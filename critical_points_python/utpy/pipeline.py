...
# Analyze an ensemble generated from sythetic dataset
# name: Name of synthetic function, ensemble, ground_truth: data, negate: negate data or not?
# n_clusters: number of local maxima in synthetic function, persistence: persistence of the
# smallest ground truth feature (hard coded above)
def analyze(
    ...
    Args:
        name (str): The name of the synthetic function.
        ...
        n_clusters (int): The number of Morse cells you expect in your
            data.
        persistence (float): The level of simplification. If known, you can set
            this to slightly smaller than half of the smallest true feature of your
            data.

    Returns:
    
        Visualizes survival and probabilistic maps for ensemble
    
    """
    
    # Negate the function values if negate flag is true
    if negate:
        ensemble = -ensemble
        if ground_truth is not None:
            ground_truth = -ground_truth

    # Name of the directory to store output images
    my_dir = "output/{}".format(name)
    if not os.path.exists(my_dir):
        os.makedirs(my_dir)

    # Save ensemble and ground truth, if ground truth is provided as input
    if ground_truth is not None:
        scipy.io.savemat(
            my_dir + "/ensemble.mat", {"ensemble": ensemble, "gt": ground_truth}
        )

    # Plot persistence chart: Map of number of clusters vs persistence
    ...
    if persistence is None and n_clusters is None:
        ...
        # Get average persistence value across ensemble members
        persistence = uu.get_persistence_from_count(ensemble, n_clusters)
    else:
        # Get average number of Morse complex cells across ensemble members
        n_clusters = uu.get_count_from_persistence(ensemble, persistence)

    # Visualize ground truth if ground truth input is provided
    if ground_truth is not None:
        # Save the ground truth Morse complex visualization in the output folder
        uv.show_msc(
            ground_truth,
            my_dir,
            persistence=None,
        ...
        # Save the function-value mapped color image to output folder
        uv.show_colormapped_image(ground_truth, my_dir, False, "gt_height.png")

    # Visualize individual ensemble members
    for i in range(ensemble.shape[2]):
        #Save Morse complex for ensemble instance to output folder
        uv.show_msc(
            ensemble[:, :, i],
            my_dir,
            persistence=None,
        ...
        #Save the function-value mapped color image for ensemble instance to output folder
        uv.show_colormapped_image(ensemble[:, :, i], my_dir, False, "realization_height_{}.png".format(i))

    # Visualize mean of all ensemble instances
    mean_realization = np.mean(ensemble, axis=2)
    # Save Morse complex for mean function to output folder
    uv.show_msc(
        mean_realization,
        my_dir,
        persistence=None,
    ...
    # Save the function-value mapped color image for mean function to output folder
    uv.show_colormapped_image(
        mean_realization, my_dir, False, "realization_mean_height.png"
    )

    # persistence chart: persistence vs. number of Morse complex cells
    # uv.show_persistence_chart(
    #    mean_realization, my_dir, False, "mean_persistence_chart.png"
    #)

    # Visualize median Morse complex visualization
    ...
    # Visualize heightmap of meadian values
    #uv.show_colormapped_image(
    #    median_realization, my_dir, False, "realization_median_height.png"
    #)

    # Show persistence chart for median values
    #uv.show_persistence_chart(
    #    median_realization, my_dir, False, "median_persistence_chart.png"
    #)

    # Visualize Survival Map
    survival_count = uv.tushar_show_survival_count(ensemble, my_dir)
    ...
    weighted_consumption_count_variance = uv.visualize_variance_weighted_consumption_count(ensemble, my_dir)
    ...
    # Map local maxima of enemble members to cluster ID, where number of clusters is equal to number of
    # mandatory maxima. Clustering is done bases on clustering algorithm in sclearn package.
    # size(maxima_map) = size(n_clusters * number of ensemble realizations)
    maxima_map = uu.create_assignment_map(
        ensemble, n_clusters=n_clusters, persistence=persistence
    )

    # Define assignments function with fixed values of parameters (maxima_map, n_clusters, and persistence)
    assignments = partial(
        uu.assign_labels,
    ...
    # Pass each ensemble realizations to newly defined assignments function, and compute probabilities
    ...
    # Visualize certain regions of probabilistic map along with 0.5 probability contours
    uv.show_contour_overlay(ensemble, assignments, my_dir, False)
    # Visualize uncertain regions of Probabilistic Map
...
# Visualize Survival and Probabilistic Map for real-world ensemble dataset
...
# Visualize Survival and Probabilistic Map for synthetic dataset
def analyze_synthetic(
    ...
    # Noise assumption in the paper: noise level <= 0.5*persistence of the smallest feature
    noise_level = 0.5 * persistence * noise_level
    
    # Generate ground truth and ensemble sampled on a 2D grid for input noise level and model
