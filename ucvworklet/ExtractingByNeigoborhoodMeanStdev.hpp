#ifndef UCV_EXTRACTING_BYNEIGHBOR_MEAN_STDEV_h
#define UCV_EXTRACTING_BYNEIGHBOR_MEAN_STDEV_h
...
public:
    ...
    VTKM_EXEC void operator()(
        ...
        OutputType &meanValue, OutputType &stdevValue, vtkm::Id workIndex) const
    {
        ...
        for (vtkm::Id k = l_k; k < r_k; k++)
        {
            for (vtkm::Id j = l_j; j < r_j; j++)
            {
                for (vtkm::Id i = l_i; i < r_i; i++)
                {
                    vtkm::Id index = k * m_xdim * m_ydim + j * m_xdim + i;
                    vtkm::FloatDefault originalvalue = inPointFieldPortal.Get(index);
                    vtkm::FloatDefault diff = originalvalue - static_cast<vtkm::FloatDefault>(meanValue);
                    diffSum += diff * diff;
                }
            }
        }
