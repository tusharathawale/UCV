...
class EntropyAdaptiveEigens : public vtkm::worklet::WorkletVisitCellsWithPoints
{
public:
    EntropyAdaptiveEigens(double isovalue, int num_sample, double thresholdInd, double thresholdEnergy)
        : m_isovalue(isovalue), m_num_sample(num_sample), m_thresholdInd(thresholdInd), m_thresholdEnergy(thresholdEnergy){};
...
private:
    ...
    double m_thresholdInd = 0.3;
    // threshold to determine the number of eigen values we should keep
    double m_thresholdEnergy = 0.01
};

#endif // EntropyAdaptiveEigens
