...
class ExtractingByNeigoborhoodEns : public vtkm::worklet::WorkletPointNeighborhood
{
public:
    ExtractingByNeigoborhoodEns(vtkm::IdComponent blockSize,
        ...
        for (vtkm::Id k = l_k; k < r_k; k++)
        {
            for (vtkm::Id j = l_j; j < r_j; j++)
            {
                for (vtkm::Id i = l_i; i < r_i; i++)
                {
                    ...
                    {
                        ensValue = inPointFieldPortal.Get(index);
                        return;
                    }
                    ensIndex++;
                }
