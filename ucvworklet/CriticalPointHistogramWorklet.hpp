#ifndef UCV_CRITICAL_POINT_HISTOGRAM_h
#define UCV_CRITICAL_POINT_HISTOGRAM_h
...
{
public:
    CriticalPointHistogramWorklet(vtkm::Id numBins) : m_numBins(numBins){};
                ...
                for (vtkm::Id k = 0; k < this->m_numBins; k++)
                {
                    for (vtkm::Id l = 0; l < this->m_numBins; l++)
                    {
                        for (vtkm::Id m = 0; m < this->m_numBins; m++)
                        {
                            ...
                            minProb = minProb + p * this->SuperOptimizedMinProb(a1, b1, a2, b2, a3, b3, a4, b4, a5, b5);
                        }
                    }
                }
            }
    ...
    VTKM_EXEC inline vtkm::FloatDefault SuperOptimizedMinProb(
        vtkm::FloatDefault a1,
                                                      ...
                                                      vtkm::Float64 w1) const
    {
        ...
        LOG(for (int i = 0; i < 6; i++)
        {
            printf("%f ", x1Limits[i]);
        })
        LOG(printf("\n");)
        LOG(printf("---debug interval\n");)
        LOG(for (int i = 0; i < 5; i++)
        {
            printf("[%f %f]", interval[i].first, interval[i].second);
        }
        printf("\n");)
...
#endif // UCV_CRITICAL_POINT_HISTOGRAM_h