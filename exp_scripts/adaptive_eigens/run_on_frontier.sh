...
RUNDIR=/lustre/orion/scratch/zw241/csc143/UCVExp
CURRDIR=$(pwd)
...
ln -s $CURRDIR/../../install_scripts/frontier_gpu/install/UCV/test_adaptive_eigen_pearson test_adaptive_eigen_pearson

EIGEN_THRESHOLD_LIST="0.0 0.05 0.10 0.15 0.20 0.25 0.30 0.35 0.40"

PEARSON_THRESHOLD_LIST="0.20 0.30 0.40 0.50"
...
for PEARSON_THRESHOLD in ${PEARSON_THRESHOLD_LIST}
do
./test_adaptive_eigen_pearson --vtkm-device kokkos $DATADIR/beetle_${DIMX}_${DIMY}_${DIMZ}_ens/ens $DATADIR/maxPearsonCorrelation.vtk ground_truth $DIMX $DIMY $DIMZ $ISO $NUM_SAMPLE $NUM_ENS beetle_${DIMX}_${DIMY}_${DIMZ}_ens_output 0.0 $PEARSON_THRESHOLD &> adaptive_pearson_eigen_kokkos_${PEARSON_THRESHOLD}.log
done
